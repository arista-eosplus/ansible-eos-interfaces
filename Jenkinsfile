...
pipeline {
    ...
    stages {
        stage ('Run tests for ansible-eos-interfaces role') {
            steps {
                    sh 'git rev-parse HEAD > revision'
                    build job: 'gar-test-starter',
                          parameters: [string(name: 'ROLE_NAME', value: 'ansible-eos-interfaces'),
